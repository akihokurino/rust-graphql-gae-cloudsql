...
pub async fn verify_id_token(token: &str) -> AppResult<String> {
...
async fn verify(
...
) -> AppResult<jsonwebtoken::TokenData<Claims>> {
    ...
    let jwks = get_firebase_jwks().await.map_err(AppError::from)?;
    ...
    jsonwebtoken::decode::<Claims>(token, &key, &validation).map_err(AppError::from)
}
